...
const roundToFluxDimension = (value: number | null | undefined) => {
  const safeValue = Number.isFinite(value) ? Math.round(value as number) : DEFAULT_OUTPUT_DIMENSION;
  const clamped = Math.max(MIN_OUTPUT_DIMENSION, Math.min(MAX_OUTPUT_DIMENSION, safeValue));
...
const getFluxCreateUrl = (model: FluxImageModel) => `${FLUX_API_BASE_URL}/${model}`;
...
export async function callFluxImageEdit(
  ...
  const resizedSecondary =
    input.secondaryImageBuffer && input.secondaryImageMimeType
      ? await sharp(input.secondaryImageBuffer)
          ...
          .toBuffer()
      : null;
  const targetMetadata = await sharp(resizedSecondary ?? resizedSource).metadata();
  ...
  const createPayload = (await createResponse.json().catch(() => null)) as FluxCreateResponse | null;
  ...
  while (Date.now() - startedAt < FLUX_POLL_TIMEOUT_MS) {
    ...
    const pollingPayload = (await pollingResponse.json().catch(() => null)) as
      | FluxPollingResponse
      | null;
    ...
    if (pollingPayload?.status === 'Ready') {
      ...
      if (!sampleUrl) {
        throw new Error('FLUX result is Ready but does not include result.sample.');
      }

      const assetResponse = await fetch(sampleUrl);

      if (!assetResponse.ok) {
        throw new Error(`FLUX result download failed with status ${assetResponse.status}.`);
      }
    ...
    if (pollingPayload?.status === 'Error') {
      throw new Error(
        extractFluxErrorMessage(pollingPayload, 'FLUX returned status Error during polling.')
      );
    }
  ...
  throw new Error(`FLUX polling timed out after ${FLUX_POLL_TIMEOUT_MS / 1000} seconds.`);
}
